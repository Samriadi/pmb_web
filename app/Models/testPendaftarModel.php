...
class testPendaftarModel
{
    ...
    public function getPendaftarTerverifikasi()
    {
        $query = "SELECT 
                        ...
                        CASE 
                            WHEN DATE_FORMAT(COALESCE(MIN(e.test_tanggal), ''), '%d-%m-%Y') = DATE_FORMAT(COALESCE(MAX(e.test_tanggal), ''), '%d-%m-%Y')
                            THEN DATE_FORMAT(COALESCE(MIN(e.test_tanggal), ''), '%d-%m-%Y')
                            ELSE CONCAT(
                    ...
                    LEFT JOIN 
                        $this->pmb_jadualtes e ON e.test_tagihanid = a.id
                    WHERE 
                        a.verified = 'Verified'
                    GROUP BY
